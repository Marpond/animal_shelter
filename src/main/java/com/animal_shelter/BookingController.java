...
public class BookingController implements Initializable {
    ...
    private ArrayList<Long> getDates(int length) {
        ...
        for (int i = 0; i <= length; i++) {
            temp.add(currentDay + ((long) i * 60 * 60 * 24));
        }
        return temp;
    }
    ...
    private void setDatesListView(int cageID) {
        ...
        for (String s : DB.returns(query)) {
            ...
            for (int i = 0; i < EPOCH_DATES.size(); i++) {
                if (formatEpochToDate(EPOCH_DATES.get(i)).equals(s.split(DB.getDELIMITER())[0])) {
                    startIndex = i;
                }
                if (formatEpochToDate(EPOCH_DATES.get(i)).equals(s.split(DB.getDELIMITER())[1])) {
                    endIndex = i;
                }
            }
            ...
            for (int i = startIndex; i <= endIndex; i++) {
                ...
                try {
                    datesListView.getItems().remove(formatEpochToDate(EPOCH_DATES.get(i)));
                }
